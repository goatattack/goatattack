...
ServerInformation::ServerInformation(hostaddr_t host, hostport_t port)
...
/* compare functions */
static bool server_info_cmp_name_asc(ServerInformation *lhs, ServerInformation *rhs) {
    return lhs->server_name < rhs->server_name;
}

static bool server_info_cmp_name_desc(ServerInformation *lhs, ServerInformation *rhs) {
    return lhs->server_name > rhs->server_name;
}

static bool server_info_cmp_locked_asc(ServerInformation *lhs, ServerInformation *rhs) {
    return lhs->secured < rhs->secured;
}

static bool server_info_cmp_locked_desc(ServerInformation *lhs, ServerInformation *rhs) {
    return lhs->secured > rhs->secured;
}

static bool server_info_cmp_players_asc(ServerInformation *lhs, ServerInformation *rhs) {
    return (lhs->max_clients - lhs->cur_clients) < (rhs->max_clients - rhs->cur_clients);
}

static bool server_info_cmp_players_desc(ServerInformation *lhs, ServerInformation *rhs) {
    return (lhs->max_clients - lhs->cur_clients) > (rhs->max_clients - rhs->cur_clients);
}

static bool server_info_cmp_ping_asc(ServerInformation *lhs, ServerInformation *rhs) {
    return lhs->ping_time < rhs->ping_time;
}

static bool server_info_cmp_ping_desc(ServerInformation *lhs, ServerInformation *rhs) {
    return lhs->ping_time > rhs->ping_time;
}
